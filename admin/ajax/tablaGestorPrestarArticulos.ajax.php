...
class TablaArticulos{
  ...
  public function mostrarTablaArticulos(){
	...
	$prestamos = ControladorArticulos::ctrMostrarPrestamos("estado", 1);
  	...
  	$datosJson = '
	 	...
	 	for($i = 0; $i < count($prestamos); $i++){
			...
			$accionesV = "<div class='btn-group'><button class='btn btn-success btnDevolverPrestamo' idPrestamo='".$prestamos[$i]["idPrestamo"]."' data-toggle='modal' data-target='#modalDevolverPrestamo'><i class='fa fa-eye'></i></button><button class='btn btn-warning btnVerPrestamo' idPrestamo='".$prestamos[$i]["idPrestamo"]."' data-toggle='modal' data-target='#modalVerPrestamo'><i class='fa fa-pencil'></i></button></div>";
